...
#include "ui_HNPreviewWidget.h"

HNPreviewWidget::HNPreviewWidget(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::HNPreviewWidget)
{
    ui->setupUi(this);
...
HNPreviewWidget::~HNPreviewWidget()
{
    delete ui;
}

int HNPreviewWidget::play()
{
    //这块代码放在哪里
    dmmu_init();
...
int HNPreviewWidget::convert_yuv_to_rgb_pixel(int y, int u, int v)
{
...
int HNPreviewWidget::convert_yuv_to_rgb_buffer(unsigned char *yuv, unsigned char *rgb, unsigned int width, unsigned int height)
{
...
void HNPreviewWidget::paintEvent(QPaintEvent *)
{
    if(fd <= 0)
        return;
...
void HNPreviewWidget::mousePressEvent(QMouseEvent *e)
{
    ...
    else
    {
        ...
        flags |= Qt::Window;
        setParent(0, flags);
        setGeometry(0, 0, HN_SCRN_WIDTH, HN_SCRN_HEIGHT);
        show();
        bFullScreen = true;
    }
