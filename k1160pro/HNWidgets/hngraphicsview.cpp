...
HNGraphicsView::HNGraphicsView(QWidget *parent) :
    QGraphicsView(parent)
{
}


void HNGraphicsView::paintEvent(QPaintEvent *e)
{
#if 1
    ...
    if(e->type() == QPaintEvent::Paint)
    {
        ...
        //p.scale(1.5/1, 1.5/1);
        HNGraphicsScene* sn = (HNGraphicsScene*)scene();
        sn->render(&p, rf, rt);
    }
    return;
#endif
...
void HNGraphicsView::drawItems(QPainter *painter, int numItems, QGraphicsItem *items[], const QStyleOptionGraphicsItem options[])
{
    for (int i = 0; i < numItems; ++i) {
        // Save painter
